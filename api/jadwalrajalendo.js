...
module.exports = async (req, res) => {
    ...
    if (req.method === 'POST') {
        try {
            ...
            return res.status(200).json({ status: true, message: "Endo OK", total: cachedData.length });
        } catch (error) {
    ...
    if (req.method === 'GET') {
        ...
        if (cachedData.length === 0) {
             return res.status(200).json({ status: true, total_data: 0, data: [], message: "Belum ada data Endodonsi." });
        }
